...
def query_groq_api(prompt: str, history: Optional[List[Dict[str, str]]] = None) -> str:
    """Queries the Groq API via direct HTTP call (bypasses SDK proxies incompatibility)."""
    api_key = os.getenv("GROQ_API_KEY")
    model_name = os.getenv("GROQ_MODEL", "llama-3.3-70b-versatile")

    if not api_key:
        logger.warning("GROQ_API_KEY environment variable is not set. Returning a mock response.")
        return f"[DEMO MODE - No Groq API Key set] Please configure GROQ_API_KEY to get live AI answers."
    ...
    try:
        ...
        # Direct REST call — no SDK, no proxies issue
